...
from utils_sim import display_sample, getK_fromAgent, sample_goal_instances_across_regions
...
def get_navigable_points_on_instances(sim, K, curr_state, depth, semantic, numSamples=20, intra_pls=False, filterByArea=False, filterInstaIDs=None, max_pl=100):
    # VLNCE env returns depth/semantic as (H, W, 1). Squeeze to (H, W) so
    # depth[row_idx, col_idx] returns (N,) not (N, 1), preventing an unintended
    # outer-product broadcast in unproject2D.
    depth    = np.asarray(depth,    dtype=np.float32).squeeze()
    semantic = np.asarray(semantic, dtype=np.int32).squeeze()
